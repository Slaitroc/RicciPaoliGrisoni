...
In this section, we will describe the interfaces of each component of the S&C platform, detailing the methods and parameters they expose and what data they return. The first image is a general overview of the components, similar to the component view, but it focuses on the interfaces between the components. The second image is a more detailed view of the interfaces of the Platform Logic components while the third image show the interfaces exposed and used by the Notification Manager.\\
In this images we tried to represent each method exposed by each component, this is why the number of methods is higher in this images than in the sequence diagrams where due to space limitation we merged some methods in a single one.\\
